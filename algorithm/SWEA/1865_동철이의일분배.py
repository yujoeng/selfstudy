...
def f(i, N, m):   # i번 사람, m i-1사람까지의 성공확률
    global max_v
    if i == N:
        if max_v < m:
            max_v = m
    elif max_v >= m:    # 아직 남은 일이 있어도 이미 성공확률이 더 낮으면 가지치지
        return
    else:
        for j in range(N):
            if used[j] == 0:   # 아직 맡은 사람이 없으면
                used[j] = 1     # i사람이 j번 일을 맡음
                f(i + 1, N, m * P[i][j] / 100)
                used[j] = 0
...
for tc in range(1, T+1):
    N = int(input())
    P = [list(map(int, input().split())) for _ in range(N)] # P[i][j]는 i번 사람이 j번 일을 성공할 확률을 퍼센트 단위로 나타낸다.

    max_v = 0   # "주어진 일이 모두 성공할 확률”의 최댓값
    used = [0] * N  # used[j] j번 일을 맡은 사람이 있는지 표시
    f(0, N, 1)
    print(f'#{tc} {max_v*100:.6f}') # 확률 -> % 표기로 변환
